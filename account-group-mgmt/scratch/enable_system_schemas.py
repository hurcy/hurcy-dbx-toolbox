...
for m in w.metastores.list():
    if m.name == 'hurcy-ap-northeast-2' and m.owner == 'cinyoung.hur@databricks.com':
        metastore_id = m.metastore_id

# print(metastore_id)
for s in w.system_schemas.list(metastore_id=metastore_id):
    print(s)
    if s.state == SystemSchemaInfoState.AVAILABLE and s.schema not in ["__internal_logging"]:
